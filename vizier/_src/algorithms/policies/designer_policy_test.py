...
class _FakeDesigner(vza.Designer):
  ...
  def __init__(self):
    self._num_incorporated_completed_trials = 0
    self._num_incorporated_active_trials = 0
    self._last_delta_completed = []
    self._last_delta_active = []
  ...
  def update(
    ...
    self._last_delta_completed = completed.trials
    self._last_delta_active = active.trials
    self._num_incorporated_completed_trials += len(completed.trials)
    self._num_incorporated_active_trials += len(active.trials)
...
class _FakeSerializableDesigner(vza.PartiallySerializableDesigner,
  ...
  def __init__(self):
    ...
    self._last_delta_completed = None

  def suggest(self,
              count: Optional[int] = None) -> Sequence[vz.TrialSuggestion]:
    return [vz.TrialSuggestion(vz.ParameterDict())] * count

  def update(self, completed: vza.CompletedTrials, active: vza.ActiveTrials):
    self._last_delta_completed = completed.trials
    self._last_delta_active = active.trials
    self._num_incorporated_completed_trials += len(completed.trials)
    self._num_incorporated_active_trials += len(active.trials)
  ...
  def dump(self) -> vz.Metadata:
    ...
    md['num_incorporated_completed_trials'] = str(
        self._num_incorporated_completed_trials
    )
    md['num_incorporated_active_trials'] = str(
        self._num_incorporated_active_trials
    )
    return md
  ...
  def recover(cls, md: vz.Metadata) -> '_FakeSerializableDesigner':
    try:
      designer = cls()
      designer._num_incorporated_completed_trials = int(
          md['num_incorporated_completed_trials']
      )
      designer._num_incorporated_active_trials = int(
          md['num_incorporated_active_trials']
      )
      return designer
  ...
  def load(self, md: vz.Metadata):
    try:
      self._num_incorporated_completed_trials = int(
          md['num_incorporated_completed_trials']
      )
      self._num_incorporated_active_trials = int(
          md['num_incorporated_active_trials']
      )
    except KeyError as e:
...
class DesignerPolicyNormalOperationTest(absltest.TestCase):
  ...
  def test_restore_fully_serializable_designer(self):
    ...
    self.assertEqual(
        getattr(designer, '_num_incorporated_completed_trials'),
        _NUM_INITIAL_COMPLETED_TRIALS,
    )
    self.assertEqual(
        getattr(designer, '_num_incorporated_active_trials'),
  ...
  def test_restore_partially_serializable_designer(self):
    ...
    self.assertLen(
        restored_policy._incorporated_active_trial_ids,
        _NUM_INITIAL_ACTIVE_TRIALS,
    )
    self.assertEqual(
        getattr(restored_policy.designer, '_num_incorporated_completed_trials'),
        _NUM_INITIAL_COMPLETED_TRIALS,
    )
    self.assertEqual(
        getattr(restored_policy.designer, '_num_incorporated_active_trials'),
        _NUM_INITIAL_ACTIVE_TRIALS,
    )
  ...
  def test_update_stateless_designer(self):
    ...
    runner.SuggestTrials(policy, 5)
    self.assertLen(
        designer._last_delta_completed,
        _NUM_INITIAL_COMPLETED_TRIALS,
    )
    self.assertLen(
        designer._last_delta_active,
        _NUM_INITIAL_ACTIVE_TRIALS,
    )
  ...
  def test_update_stateful_designer(self):
    ...
    self.assertLen(designer._last_delta_completed, 33)
    # Check that the newly active trials are passed.
    self.assertLen(designer._last_delta_active, 11)
